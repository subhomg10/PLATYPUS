...
def search_series(title):
    query = '''SELECT title, description, genre, language, released_on, poster
    FROM SERIES WHERE title ILIKE %s ;'''
    with get_connection() as con:
...
def get_top_rated_series(count):
    query = 'SELECT id, title, rating FROM SERIES ORDER BY RATING DESC LIMIT %s ;'
    with get_connection() as con:
        with con.cursor(cursor_factory = psycopg2.extras.DictCursor) as cur:
...
def get_popular_series(count):
    query = 'SELECT id, title, rating FROM SERIES ORDER BY POPULARITY DESC LIMIT %s ;'
    with get_connection() as con:
        with con.cursor(cursor_factory = psycopg2.extras.DictCursor) as cur:
            cur.execute(query, (count,))
            return cur.fetchall()
...
def get_series_by_genre(genre, count):
    query = 'SELECT id, title, rating FROM SERIES WHERE genre ILIKE %s ORDER BY RATING DESC LIMIT %s ;'
    with get_connection() as con:
        with con.cursor(cursor_factory = psycopg2.extras.DictCursor) as cur:
...
def get_series_by_language(language, count):
    query = 'SELECT id, title, rating FROM SERIES WHERE language ILIKE %s ORDER BY RATING DESC LIMIT %s ;'
    with get_connection() as con:
        with con.cursor(cursor_factory = psycopg2.extras.DictCursor) as cur:
...
def get_top_rated_movies(count):
    query = 'SELECT id, title, rating FROM MOVIES ORDER BY RATING DESC LIMIT %s ;'
    with get_connection() as con:
        with con.cursor(cursor_factory = psycopg2.extras.DictCursor) as cur:
            cur.execute(query, (count,))
            return cur.fetchall()
...
def get_popular_movies(count):
    query = 'SELECT id, title, rating FROM MOVIES ORDER BY POPULARITY DESC LIMIT %s ;'
    with get_connection() as con:
        with con.cursor(cursor_factory = psycopg2.extras.DictCursor) as cur:
            cur.execute(query, (count,))
            return cur.fetchall()
...
def get_movies_by_genre(genre, count):
    query = 'SELECT id, title, rating FROM MOVIES WHERE genre ILIKE %s ORDER BY RATING DESC LIMIT %s ;'
    with get_connection() as con:
        with con.cursor(cursor_factory = psycopg2.extras.DictCursor) as cur:
            cur.execute(query, (f'%{genre}%', count))
            return cur.fetchall()
...
def get_movies_by_language(language, count):
    query = 'SELECT id, title, rating FROM MOVIES WHERE language ILIKE %s ORDER BY RATING DESC LIMIT %s ;'
    with get_connection() as con:
        with con.cursor(cursor_factory = psycopg2.extras.DictCursor) as cur:
            cur.execute(query, (f'%{language}%', count))
            return cur.fetchall()
...
def get_all_genres_movies():
    query = 'SELECT DISTINCT unnest(genre) AS genre FROM MOVIES ORDER BY genre ;'
    with get_connection() as con:
        with con.cursor() as cur:
            cur.execute(query)
...
def get_all_genres_series():
    query = 'SELECT DISTINCT unnest(genre) AS genre FROM SERIES ORDER BY genre ;'
    with get_connection() as con:
        with con.cursor() as cur:
            cur.execute(query)
            genres = [row[0] for row in cur.fetchall()]
...
def get_all_language_movies():
    query = 'SELECT DISTINCT unnest(language) AS language FROM MOVIES ORDER BY language ;'
    with get_connection() as con:
        with con.cursor() as cur:
            cur.execute(query)
...
def get_all_language_series():
    query = 'SELECT DISTINCT unnest(language) AS language FROM SERIES ORDER BY language ;'
    with get_connection() as con:
        with con.cursor() as cur:
            cur.execute(query)
            language = [row[0] for row in cur.fetchall()]
